<template>
  <div class="custom-upload-content">
    <Button :disabled="disabled" :id="`CustomUpload${idIndex}`" class="custom-btn-upload" :class="className" :type="btnType" ref="uploadBtn">{{showBtnText}}</Button>
    <p v-if="showProgress && isProgress" class="info">总大小：{{totalSize+'mb'}}，进度：{{progress+'%'}}</p>
  </div>
</template>
<script>
export default {
    ...
    uploadInit() {
      ...
      this.uploader = new plupload.Uploader({
        ...
        filters: {
          ...
          mime_types: [
            { title: 'Format files', extensions: that.format }
          ],
